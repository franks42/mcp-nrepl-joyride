...
run_test "Server initialization" \
    'uv run python scripts/stdio_mcp_client.py --server-cmd "bb -cp src src/mcp_server/core.clj" --test-basic --quiet' \
    "Basic tests passed"
...
run_test "Simple arithmetic (+ 1 2 3)" \
    'uv run python scripts/stdio_mcp_client.py --server-cmd "bb -cp src src/mcp_server/core.clj" --tool debug-eval --args '"'"'{"code": "(+ 1 2 3)"}'"'"' --quiet' \
    'result.*:.*6'

# Test 4: Variable definition
run_test "Variable definition" \
    'uv run python scripts/stdio_mcp_client.py --server-cmd "bb -cp src src/mcp_server/core.clj" --tool debug-eval --args '"'"'{"code": "(def test-var 42)"}'"'"' --quiet' \
    "test-var"

# Test 5: Function definition and call
run_test "Function definition and call" \
    'uv run python scripts/stdio_mcp_client.py --server-cmd "bb -cp src src/mcp_server/core.clj" --tool debug-eval --args '"'"'{"code": "(do (defn square [x] (* x x)) (square 5))"}'"'"' --quiet' \
    'result.*:.*25'

# Test 6: Data structures
run_test "Data structures (vector)" \
    'uv run python scripts/stdio_mcp_client.py --server-cmd "bb -cp src src/mcp_server/core.clj" --tool debug-eval --args '"'"'{"code": "[1 2 3 4 5]"}'"'"' --quiet' \
    'result.*\[1 2 3 4 5\]'

# Test 7: Map operations
run_test "Map operations" \
    'uv run python scripts/stdio_mcp_client.py --server-cmd "bb -cp src src/mcp_server/core.clj" --tool debug-eval --args '"'"'{"code": "(:name {:name \"test\" :value 42})"}'"'"' --quiet' \
    'result.*test'

# Test 8: Error handling
run_test "Error handling (division by zero)" \
    'uv run python scripts/stdio_mcp_client.py --server-cmd "bb -cp src src/mcp_server/core.clj" --tool debug-eval --args '"'"'{"code": "(/ 1 0)"}'"'"' --quiet' \
    'status.*error'

# Test 9: Empty code parameter
run_test "Empty code parameter validation" \
    'uv run python scripts/stdio_mcp_client.py --server-cmd "bb -cp src src/mcp_server/core.clj" --tool debug-eval --args '"'"'{"code": ""}'"'"' --quiet' \
...
run_test "Load test-toolkit.clj" \
    'uv run python scripts/stdio_mcp_client.py --server-cmd "bb -cp src src/mcp_server/core.clj" --tool debug-load-file --args '"'"'{"file-path": "test-toolkit.clj"}'"'"' --quiet' \
    'successful.*4'

# Test 11: File not found error
run_test "File not found error" \
    'uv run python scripts/stdio_mcp_client.py --server-cmd "bb -cp src src/mcp_server/core.clj" --tool debug-load-file --args '"'"'{"file-path": "non-existent-file.clj"}'"'"' --quiet' \
    'File not found'

# Test 12: Empty file-path parameter
run_test "Empty file-path parameter validation" \
    'uv run python scripts/stdio_mcp_client.py --server-cmd "bb -cp src src/mcp_server/core.clj" --tool debug-load-file --args '"'"'{"file-path": ""}'"'"' --quiet' \
...
run_test "MCP introspection via debug-load-file" \
    'uv run python scripts/stdio_mcp_client.py --server-cmd "bb -cp src src/mcp_server/core.clj" --tool debug-load-file --args "{\"file-path\": \"mcp-introspection-tests.clj\"}" --quiet' \
    'successful.*14'

# Test 14: Check namespace functionality
run_test "Current namespace check" \
    'uv run python scripts/stdio_mcp_client.py --server-cmd "bb -cp src src/mcp_server/core.clj" --tool debug-eval --args "{\"code\": \"(str *ns*)\"}" --quiet' \
    'mcp-server.core'

# Test 15: Create and use simple tool registry
run_test "Simple tool registry creation" \
    'uv run python scripts/stdio_mcp_client.py --server-cmd "bb -cp src src/mcp_server/core.clj" --tool debug-eval --args "{\"code\": \"(keys {\\\"debug-eval\\\" 1 \\\"debug-load-file\\\" 2})\"}" --quiet' \
    'debug-eval.*debug-load-file'

# Test 16: Server state atom simulation
run_test "Server state atom simulation" \
    'uv run python scripts/stdio_mcp_client.py --server-cmd "bb -cp src src/mcp_server/core.clj" --tool debug-eval --args "{\"code\": \"(let [state (atom {:connected true})] @state)\"}" --quiet' \
    'connected.*true'

# Test 17: Tool invocation simulation
run_test "Tool invocation simulation" \
    'uv run python scripts/stdio_mcp_client.py --server-cmd "bb -cp src src/mcp_server/core.clj" --tool debug-eval --args "{\"code\": \"(defn mock-tool [name] {:tool name :status \\\"success\\\"}) (mock-tool \\\"debug-eval\\\")\"}" --quiet' \
    'tool.*debug-eval'
...
run_test "Advanced file loading with calculations" \
    'uv run python scripts/stdio_mcp_client.py --server-cmd "bb -cp src src/mcp_server/core.clj" --tool debug-load-file --args '"'"'{"file-path": "/tmp/phase1-test.clj"}'"'"' --quiet' \
    'successful.*4'

# Test 19: State persistence across tool calls
run_test "State persistence test (define var)" \
    'uv run python scripts/stdio_mcp_client.py --server-cmd "bb -cp src src/mcp_server/core.clj" --tool debug-eval --args '"'"'{"code": "(def persistent-var :phase1-test)"}'"'"' --quiet' \
    "persistent-var"
