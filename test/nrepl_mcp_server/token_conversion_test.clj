...
(deftest token-amount-validation
  ...
  (testing "Invalid token amounts"
    ...
    (is (false? (calc/token-amount? [1000 :hash])))  ; Keyword instead of string
...
(deftest token-amount-accessors
  ...
  (testing "get-unit extracts unit"
    (is (= "hash" (calc/get-unit [1000 "hash"])))
    (is (= "usd" (calc/get-unit [0.032 "usd"]))))

  (testing "token-amount constructor"
    (is (= [1000 "hash"] (calc/token-amount 1000 "hash")))
    (is (= [0.032 "usd"] (calc/token-amount 0.032 "usd")))))
