...
async def test_failing_call():
    ...
    async with httpx.AsyncClient() as client:
        # Connect first
        print("=== CONNECTING ===")
        response = await client.post("http://localhost:3000/mcp/", json=connect_payload, headers=headers)
        print(json.dumps(response.json(), indent=2))
        
        # Try the failing call
        print("\n=== TRYING FAILING CALL ===")
        response = await client.post("http://localhost:3000/mcp/", json=failing_payload, headers=headers)
